...
## 4. Структура файлов (Обновлено 2026-06-13)
...
- `/app/api/admin-test/route.ts` - API-роут для обработки результатов теста и отправки отчета по Email (через nodemailer).
- `/lib/telegram.ts` - Вспомогательные функции для интеграции с Telegram Bot API (добавлена поддержка переопределения токена и чата, а также пересылки через прокси openclaw).
- `/lib/email.ts` - Утилита для отправки писем через SMTP (используется для уведомлений о тестах).
...
- `/components/HeroNew.tsx` - Мульти-хиро блок с отзывчивой анимацией по скроллу.
- `/components/ZonesPreview.tsx` - Компонент предпросмотра зон с Tinder-style свайпами и интуитивными подсказками.
...
- **2026-06-07**: Внедрено улучшение технического теста администраторов (`/admin-test`). Разработан серверный API-роут `/api/admin-test/route.ts` для отправки результатов в рабочий Telegram-чат/канал, но из-за блокировки API на хостинге был развернут микро-прокси на сервере openclaw. Из-за ненадежности Telegram маршрутизация переведена на Email (добавлен пакет `nodemailer` и утилита `/lib/email.ts`). В `.env.local` добавлены настройки `SMTP_HOST`, `SMTP_PORT`, `SMTP_USER`, `SMTP_PASS` (используется пароль приложения Google) и `ADMIN_EMAIL`. Настроен сбор логов ошибок администраторов. Добавлен чекбокс обязательного согласия с правилами перед началом теста в `/app/admin-test/page.tsx`. Порог успешной сдачи изменен до 4 ошибок. Обновлен интерфейс правил (`/admin-rules/AdminRulesClient.tsx`) и улучшена механика свайпов (Tinder-style) в компоненте `ZonesPreview.tsx`.
- **2026-06-13**: Улучшена отзывчивость мульти-хиро блока (`HeroNew.tsx`) к скроллу на мобильных устройствах за счет уменьшения высоты scroll-контейнера (`h-[300svh]`). В компоненте `ZonesPreview.tsx` полностью переработана механика (отказ от Tinder-стиля свайпов): внедрена горизонтальная sticky-скролл галерея, позволяющая просматривать все зоны сразу с интересной анимацией карточек без необходимости явных свайпов или лайков.
