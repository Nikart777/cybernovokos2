...
const MOCK_POSTS: TelegramPost[] = [
    {
        id: 'mock_1',
        text: '🎉 Новый игровой турнир по CS2 стартует в эту субботу! Призовой фонд 50 000 ₽',
...
async function fetchTelegramPosts(): Promise<{ posts: TelegramPost[]; source: 'telegram' | 'mock' }> {
    const CHANNEL_ID = process.env.TELEGRAM_NEWS_CHANNEL_ID || 'cyberxn32';

    // Clean handle
    const cleanHandle = CHANNEL_ID.replace('@', '').split('/').pop() || 'cyberxn32';

    const RSS_URL = `https://tg.i-c-a.su/rss/${cleanHandle}`;

    console.log('\n========== NEWS RSS FEED DEBUG ==========');
    console.log('[News RSS] Starting fetch process...');
    console.log('[News RSS] Channel Handle: ' + cleanHandle);
    console.log('[News RSS] RSS URL: ' + RSS_URL);
    ...
    try {
        ...
        console.log(`[News RSS] Response status: ${response.status}`);
        ...
        console.log(`[News RSS] ✅ RSS XML received (${xmlText.length} bytes)`);
        ...
        if (posts.length === 0) return { posts: MOCK_POSTS, source: 'mock' };
...
export async function GET() {
    try {
        const now = Date.now();
        const { posts, source } = await fetchTelegramPosts();
        ...
        return NextResponse.json(
            { error: 'Failed to fetch News posts', posts: MOCK_POSTS, source: 'mock' },
            { status: 500 }
        );
    }
}
