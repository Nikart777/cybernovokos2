...
- Отдельная "полная регистрация" для оплаты бонусами не требуется.

Ограничения по списанию бонусов:
- Бонусами можно оплатить до 20 процентов стоимости тарифов 1 час, 3 часа и 5 часов.
- Тариф "Ночь" бонусами не оплачивается: на ночные пакеты списание бонусов не действует.
- 1 бонус = 1 рубль.
...
Важно: акция работает только начиная со статуса "Серебро". Гостям со статусом "Бронза" бонус за пополнение не начисляется.
- При пополнении от 1000 ₽ начисляется 100 бонусов - для статусов "Серебро" и выше.
- При пополнении от 2000 ₽ начисляется 250 бонусов - для статусов "Золото" и выше.
- При пополнении от 3000 ₽ начисляется 350 бонусов - для статусов "Платина" и выше.
